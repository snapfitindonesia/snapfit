  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6">
      {banners.map((b, idx) => (
        ...
        >
          <Image src={b.image} alt={`Promo SNAPFIT ${idx + 1}`} fill sizes="(max-width:640px) 100vw, 600px" className="object-cover transition-transform duration-500 hover:scale-105" />
        </Link>
      ))}
    </div>
  );
...
export function StripBanner({ banner }: { banner: MainBanner | null }) {
  ...
  return (
    <Link href={banner.href} className="block overflow-hidden rounded-xl border border-border">
      ...
      <img src={banner.image} alt="Promo SNAPFIT" width={2000} height={400} loading="lazy" decoding="async" className="h-auto w-full object-cover" />
    </Link>
  );
}
